...
## 2026-04-06 - Stable Prop Callbacks and React.memo for List Items
**Learning:** In React, mapping list items as inline elements (like `<label>` and `<input>`) causes React to re-render the entire list (every element) whenever the parent component updates (such as a selection change for a single item). If we extract the list item into a separate component wrapped in `React.memo`, we must ensure that all callbacks passed to it are perfectly stable. When a callback (like `toggleSkill`) depends on state from props (like `selectedSkills`), it forces the callback to be recreated on every selection, defeating the memoization.
**Action:** Use a `useRef` to hold the latest prop values (e.g., `selectedSkillsRef.current = selectedSkills;`) and read from the ref inside the `useCallback` instead of declaring the prop in the dependency array. This guarantees the callback reference never changes, allowing `React.memo` on the child items to work perfectly, reducing O(N) re-renders to O(1) when toggling an item.
