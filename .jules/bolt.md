...
## 2026-05-29 - Prevent Render Bypasses by Memoizing Top-level Forms
**Learning:** In React applications, even if child components within a complex view (like `SelectField` inside `ProjectForm`) are wrapped in `React.memo`, if the parent view component (`ProjectForm`) is not memoized and receives unstable function props from its parent (`App.js`), it will be forced to re-render whenever the very top-level application state changes (e.g., polling `apiStatus`). This creates unnecessary overhead as React still has to traverse and diff the intermediate `ProjectForm` fiber tree.
**Action:** Use `React.useCallback` to stabilize event handlers in top-level components (like `App.js`) and ensure immediate children (`ProjectForm`) are wrapped in `React.memo` to completely halt the render propagation early in the tree.
