...
## 2026-06-25 - Stabilize Props Before Adding React.memo
**Learning:** Wrapping a large, expensive component (like `ProjectForm`) in `React.memo` provides no benefit if the parent component (`App.js`) passes inline or unstable callback functions to it (e.g. `<ProjectForm onSubmit={(data) => setProjectDataForAgents(data)} />`). This defeats the shallow comparison check of `React.memo` and causes the child component to re-render anyway.
**Action:** Before optimizing a component with `React.memo`, always verify and stabilize the props passed to it by the parent component using `useCallback` or by passing the stable `setState` dispatcher directly.
